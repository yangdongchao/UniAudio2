...
class Generator:
    ...
    def prepare_AudioGen_task_for_cfg(self, task_prompt, text_seq):
        ...
        text_seq = self.add_special_token('caption_seq', text_seq) 
    ...
    def prepare_AudioGen_task(self, task_prompt, text_seq):
        sequence, seq_mask = [], []
        this_prompt_data = self.text_pad(task_prompt)
        this_prompt_mask = torch.zeros((this_prompt_data.shape[0], self.parallel_number))
        this_prompt_mask[:,-1] = True
    ...
    def generate_audio(
        ...
        tokens, tokens_mask = self.prepare_AudioGen_task(task_prompt, text_token)
        if self.is_cfg:
            cfg_prompt_tokens, cfg_prompt_tokens_mask = self.prepare_AudioGen_task_for_cfg(task_prompt, text_token)
...
if __name__ == '__main__':
    ...
    text_dict = torch.load(data_content['keys']['caption_seq'], map_location='cpu')
    ...
    for key in semantic_dict.keys():
        ...
        de_reason_tokens, de_semantic_tokens = generator.generate_audio(task_name=data_content['task'], task_prompt = tmp_task_prompt,
