# build the Instruct TTS task evlaution set
...
class Generator:
    ...
    def prepare_instruct_tts_task_for_cfg(self, task_prompt, caption_seq, text_seq):
        ...
        # add the caption
        caption_seq = self.add_special_token('caption_seq', caption_seq)
        caption_seq = torch.ones_like(caption_seq)*self.text_pad_token # replace the caption sequence
        this_caption_data = self.text_pad(caption_seq)
        this_caption_mask = torch.zeros((this_caption_data.shape[0], self.parallel_number))
        this_caption_mask[:,-1] = True
        ...
        this_data = torch.cat([this_prompt_data, this_caption_data,  this_text_data], dim=0)
        this_mask = torch.cat([this_prompt_mask, this_caption_mask,  this_text_mask], dim=0)
        return this_data, this_mask


    def prepare_instruct_tts_task(self, task_prompt, caption_seq, text_seq):
        sequence, seq_mask = [], []
        this_prompt_data = self.text_pad(task_prompt)
        this_prompt_mask = torch.zeros((this_prompt_data.shape[0], self.parallel_number))
        this_prompt_mask[:,-1] = True
    ...
    def generate_instruct_tts(
        ...
        tokens, tokens_mask = self.prepare_instruct_tts_task(task_prompt, caption, text_token)
        if self.is_cfg:
            cfg_prompt_tokens, cfg_prompt_tokens_mask = self.prepare_instruct_tts_task_for_cfg(task_prompt, caption, text_token)
...
if __name__ == '__main__':
    ...
    caption_dict = torch.load(data_content['keys']['caption_seq'], map_location='cpu')
    if data_content['task'] == 'speech_sound_generation':
        task_seqs  = task_prompts['InstructTTS']
    else:
    ...
    for key in semantic_dict.keys():
        ...
        de_reason_tokens, de_semantic_tokens = generator.generate_instruct_tts(task_name=data_content['task'], 
                                    task_prompt = tmp_task_prompt,
                                    text_token = tmp_text, caption=tmp_caption, 
                                    semantic_token = tmp_semantic_token, reason_token= tmp_reason_token,
