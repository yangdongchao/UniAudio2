# build the zero-shot understanding task evaluation code
...
def check_token_range_simple(curr_tokens, vocab_size):
    """
    简化版检查：只检查最大值是否超出范围
    """
    import sys
    ...
    max_token = curr_tokens.max().item()
    min_token = curr_tokens.min().item()
    
    print(f"Token范围检查:")
    print(f"  词表大小: {vocab_size}")
    print(f"  最小token值: {min_token}")
    print(f"  最大token值: {max_token}")
    
    if min_token < 0:
        print(f"❌ 发现负值token: {min_token}")
        print(f"   通常token值应该 >= 0")
        sys.exit(1)
    
    if max_token >= vocab_size:
        print(f"❌ 发现超出词表的token!")
        print(f"   最大token值 {max_token} >= 词表大小 {vocab_size}")
        print(f"   超出范围: {max_token - vocab_size}")
        
        # 找出所有超出范围的token
        out_of_range = curr_tokens >= vocab_size
        if out_of_range.any():
            invalid_count = out_of_range.sum().item()
            invalid_values = curr_tokens[out_of_range].unique()
            print(f"   共有 {invalid_count} 个超出范围的token")
            print(f"   无效token值: {invalid_values.tolist()}")
...
class Generator:
    ...
    def get_condition_seq(self, d, keys, types, task_prompt_data):
        ...
        for key, tp in zip(keys, types):
            if tp == 'text': 
                ...
                this_data = d[key].long()
                if this_data.dim() == 2 and this_data.shape[0] == 8 and this_data.shape[1] != 8:
                    this_data = this_data.transpose(0, 1)
            if tp == 'text':
    ...
    def generate_answer(
        ...
        if curr_tokens.shape[1] >= 1500:
            return -1,-1
        # pre-filling
        _ = self._model.forward_prefix(curr_tokens[:,:-1], labels=curr_tokens[:,1:,:-1], tokens_mask=curr_tokens_mask, 
                                      loss_mask=curr_tokens_mask, input_pos=curr_pos[:,:-1], input_pos_maxp1=prompt_tokens.size(0)-1)
        ...
        return text_content, curr_tokens.shape[1]
...
if __name__ == '__main__':
    ...
    for key in data_dict.keys():
        ...
        text_content, seq_len = generator.generate_answer(task_name=d['task'], task_prompt = tmp_task_prompt,
                                    d=d, keys=task_format['keys'][:-1], types=task_format['type'][:-1],
                                    temperature = args.temperature, topk = args.topk, cfg_scale=args.cfg_scale)
        f_out.write(key+'\t'+text_content+'\n')
