# build the speech edit (including denoise and edit acoustic information) task evaluation code
...
if __name__ == '__main__':
    ...
    for key in data_dict.keys():
        cnt += 1
        tmp_task_prompt = task_seqs[1] # 
        d = data_dict[key] # samples
        print(d['task'])
        if d['task'] == 'speech_edit':
        ...
        source_semantic_tokens = d['semantic_seq_1']
        target_semantic_tokens = d['semantic_seq_2']
        ...
        torch.save(source_semantic_tokens.detach().cpu(), f'{args.output_dir}/{key}_semantic1_gt.pt')
        torch.save(target_semantic_tokens.detach().cpu(), f'{args.output_dir}/{key}_semantic2_gt.pt')
        #assert 1==2
