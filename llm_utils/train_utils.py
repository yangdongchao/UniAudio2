...
from llm_utils.abs_scheduler import AbsBatchStepScheduler
...
def maybe_resume_checkpoint(args, model, optimizer, scheduler, reporter, train_dl):
    ...
    use_resume = False
    if len(ckpts) != 0 and args.resume is not None:
    ...
    elif args.resume is not None and Path(args.resume).is_file():
    ...
    if use_resume == False:
        # use_resume 为False意味着我们不是手动指定之前train好的模型。而是继续之前的exp路径训练. 所以需要load optimizer
        # 反过来，如果use_resume=True. 那代表我们load warm-up阶段的模型。不用更新optimizer
...
def resume_for_inference(resume, exp_dir, model, device='cpu'):
