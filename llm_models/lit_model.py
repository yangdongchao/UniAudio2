...
class GPT(nn.Module):
    ...
    def forward(
        ...
        """
        x: we change it as the continous feature: (B, T, D)
    ...
    def reset_kv_cache(self, zero_memory: bool = True) -> None:
        """Reset KV caches in-place without freeing the allocated buffers."""
        for block in self.transformer.h:
            cache = getattr(block.attn, "kv_cache", None)
            if isinstance(cache, KVCache):
                if zero_memory:
                    # 清零 K/V buffer（KVCache 已提供 reset_parameters）
                    cache.reset_parameters()
                else:
                    # 仅“逻辑”重置（通常已足够，后续用 input_pos 覆盖写）
